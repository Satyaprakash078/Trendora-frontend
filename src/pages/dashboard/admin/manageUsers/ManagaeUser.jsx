...
const ManagaeUser = () => {
    ...
    const handleDeleteUser=async (id)=>{
        try {
           ...
           console.log("Failed to dlete user",error) 
        }
    }
                                        ...
                                        users && users.map((user, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="border-t-0 px-6 align-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                    <span className={`rounded-full px-2 py-[2px] ${user?.role === 'admin' ? 'bg-indigo-500 text-white':'bg-amber-300'}`}>
                                                        {user?.role}</span>
                                                </td>
